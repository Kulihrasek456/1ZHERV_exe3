...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        Vector3 positionDifference = mRigidBody.position - nearestPlayer.transform.position;
        mRigidBody.rotation = Quaternion.LookRotation(positionDifference, new Vector3(0,0,1));
        Vector3 positionChange = Vector3.Normalize(positionDifference)*speed*Time.deltaTime;

        if(Vector3.Magnitude(positionDifference) > 0.2f){
            mRigidBody.MovePosition(mRigidBody.position - positionChange);
        } 
    }
