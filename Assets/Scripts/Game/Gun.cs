...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        var newX =  director.position.x;
        var newY =  director.position.y;
        var newZ =  director.position.z;

        if(shotgun){
            float bulletAngleDiff = shotgunSpread/shotgunBullets;
            float startingAngle = -(shotgunSpread/2)+(bulletAngleDiff/2);
            for (int i = 0; i<shotgunBullets; i++){
                float currentAngleChange = (bulletAngleDiff*i)+startingAngle;
                SpawnBullet(
                    new Vector3{ x =newX, y = newY, z = newZ }, 
                    Quaternion.Euler(0.0f, 0.0f, director.eulerAngles.z+currentAngleChange)
                );
            }
        }else{
            SpawnBullet(
                new Vector3{ x =newX, y = newY, z = newZ }, 
                Quaternion.Euler(0.0f, 0.0f, director.eulerAngles.z)
            );
        }
    }
